...
public:
	...
	virtual void OnUpdate() override
	{
		for (int i = 0; i < 120; i++)
		{
			...
			////
				...
				velocity = Alalba::VelocityNoise < lux::Vector >(parm, dx, dy, dz, 1.5*i);
				velocity = Alalba::Grid < lux::Vector >(lux::Vector(0.0, 0.0, 0.0), lux::Vector(4.0, 4.0, 4.0), { 513,513,513 }, 4, velocity);
				headDensity = ajax_grid;
			...
			elapsed = std::chrono::duration_cast<std::chrono::seconds>(end - startframe).count();
	...
	virtual void OnInit() override
	{
		...
		m_renderer.reset(new Alalba::Renderer(1980, 1080, rayStepSize));
		...
		velocity = Alalba::VelocityNoise < lux::Vector > (parm, dx, dy, dz, 2.5);
		velocity = Alalba::Grid < lux::Vector > (lux::Vector(0.0, 0.0, 0.0), lux::Vector(4.0, 4.0, 4.0), { 513,513,513 }, 4, velocity);
